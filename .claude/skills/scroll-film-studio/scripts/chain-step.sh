#!/bin/zsh
# chain-step.sh <assets-dir> <clip-name> <start-image> <prompt> [prev-last-png] [resolution] [end-image]
#
...
#
# PASS <end-image> WHENEVER YOU HAVE THE NEXT KEYFRAME. Seedance 2.0 takes end_image
# (verified against the live CLI, 25 Jul 2026) and a clip pinned at both ends is FORCED
# to land on the next keyframe. Pinned at the start only, the model is free to wander and
# then cut back to where it needs to be — and that cut is the jump the viewer sees. This
# script chained on the start image alone until 25 Jul 2026 because the skill wrongly
# recorded Seedance as start-image-only; that single false line is the root of the
# jump-cuts in the 25 Jul films.
set -e -o pipefail
DIR=$1; NAME=$2; START=$3; PROMPT=$4; PREV=$5; RES=${6:-1080p}; END=${7:-}
if [[ -z "$DIR" || -z "$NAME" || -z "$START" || -z "$PROMPT" ]]; then
  echo "usage: chain-step.sh <assets-dir> <clip-name> <start-image> <prompt> [prev-last-png] [resolution] [end-image]"; exit 1
fi
[[ -r "$START" ]] || { echo "start image not readable: $START"; exit 1; }
if [[ -n "$END" ]]; then
  [[ -r "$END" ]] || { echo "end image not readable: $END"; exit 1; }
else
  echo "[$NAME] WARNING: no end-image given — this clip is pinned at ONE end and is free"
  echo "[$NAME] to drift and cut back. Pass the next keyframe as argument 7 unless this is"
  echo "[$NAME] genuinely the final clip of the film."
fi
