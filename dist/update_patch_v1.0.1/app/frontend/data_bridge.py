...
def get_lead_growth_data() -> tuple:
    """Return (months, new_leads, cumulative) for the lead growth chart."""
    from app.database.db import get_conn
    _seed_if_empty()
    try:
        with get_conn() as conn:
            rows = conn.execute(
                ...
                "FROM leads GROUP BY month ORDER BY month ASC LIMIT 12"
            ).fetchall()
        if rows:
            months = [r["month"] for r in rows]
            new_leads = [r["cnt"] for r in rows]
        else:
            # Fallback: generate 6-month mock series
            from datetime import date, timedelta
            today = date.today()
            months = [(today.replace(day=1) - timedelta(days=30 * i)).strftime("%Y-%m") for i in range(5, -1, -1)]
            new_leads = [2, 3, 5, 4, 6, 8]
        cumulative = []
        total = 0
        for n in new_leads:
            total += n
            cumulative.append(total)
        return months, new_leads, cumulative
    except Exception:
        months = ["2025-11", "2025-12", "2026-01", "2026-02", "2026-03", "2026-04"]
        new_leads = [2, 3, 5, 4, 6, 8]
        cumulative = [2, 5, 10, 14, 20, 28]
        return months, new_leads, cumulative
